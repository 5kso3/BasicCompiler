...
public class TypeChecker {
    ...
    static Type Check(Assignment a, TypeEnv te) {
    	if(!te.contains(a.id)) {
    		error(a, " undefined variable in assgnment: "+a.id);
    		return Type.ERROR;
    	}
    	Type t1 = te.get(a.id);
    ...
    static Type Check(For l, TypeEnv te) {
    	Type t = Type.ERROR;
    	addType(new Decls(new Decl(l.id1.toString(), l.type, l.expr1)), te);
        if(!te.contains(l.id1)) {
        	error(l, " undefined variable in assignment: " + l.id1);
        	return Type.ERROR;
        }
        if(l.expr1 != null) {
        	if(l.type != Check(l.expr1, te)) {
        		error(l, "type error :  mixed mode assignment to " + l.id1);
        	}
        }
        if(Check(l.expr2, te) == Type.BOOL) {
        	if(Check(l.stmt, te) == Type.VOID) {
        		t = Type.VOID;
        	} else {
        		error(l, "return in loop..");
        	}
        ...
        if(!(Check(l.id2, te) == Check(l.expr3, te))) {
        	error(l, "type error :  mixed mode assignment to " + l.id2);
        }
        return t;
    }
