...
public class Sint {
    ...
    State Eval(For f, State state) {
    	// for문의 state관리는 let문을 참고하여 만들었습니다
    	Decls d = new Decls(new Decl(f.id1.toString(), f.type ,f.expr1));
    	State s = allocate(d, state);
    	state.push(f.id1, V(f.expr1, state));
    	//for문 자체를 재귀하는 것이 아닌 반복문 안에서 부분적으로 재귀를 사용하였습니다
    	while(V(f.expr2, state).boolValue()) {
    		Eval(f.stmt, state);
    		Eval(f.id2, f.expr3, state);
    	}
    	return free(d, s);
    }
    
    private void Eval(Identifier id, Expr expr, State state) {
		state.set(id, V(expr, state));
	}
